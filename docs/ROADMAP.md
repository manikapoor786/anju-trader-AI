...
| 1.5 | Cut any score bucket with cost-adjusted expectancy ≤ 0 | 🟡 Re-scoped — see [PHASE_1_5_PLAN.md](PHASE_1_5_PLAN.md) — bucket-cutting doesn't help (all negative); real fix is two-stage exit + entry-model filter |
...
| 3.3 | Manual approve/reject via GitHub mobile workflow_dispatch → applies revision | ✅ Done | 2026-05-14 · 14 unit tests · PARAMETER changes auto-applied to model_params.json · WEIGHT changes need manual code PR · backtest-required revisions block approval |
...
| 3.6 | Reasoning-trace audit UI (simple Telegram report from memory.db) | ✅ Done | 2026-05-14 · 8 unit tests · 6 aggregators (signals/outcomes/llm/lessons/revisions/anomalies) · audit_report.yml Saturday cron + manual |
